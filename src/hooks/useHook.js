...
/* function that returns an array of plants that should populate for a specific day */
  const dailyPlants = (targetDate) => {
    ...
    for (let i =0 ; i < plantData.length ; i++) {
      ...
      const watering = (((targetDate - plantData[i].lastWaterDate) / oneDay % plantData[i].waterFrequency) === 0) || (((targetDate - plantData[i].lastWaterDate + 3600) / oneDay % plantData[i].waterFrequency) === 0) || (((targetDate - plantData[i].lastWaterDate - 3600) / oneDay % plantData[i].waterFrequency) === 0)
